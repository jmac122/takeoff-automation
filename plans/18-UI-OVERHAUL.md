...
> **Updated:** February 5, 2026 (audit review — added architecture sections, missing tasks, server sync)
...
**Boundary Rule:** Server data (sheets, conditions, measurements) → **React Query cache**. Zustand holds only **UI state** (selections, active items, viewport, drawing mode).
...
| StatusBar | workspaceStore (zoom, cursor, tool, selection) — READ ONLY | Nothing |
...
**Failure handling:** POST failures → keep geometry in `currentPoints` for retry. DELETE failures → restore in cache (optimistic rollback). All failures → toast with error.
...
| AI prediction failure | Silent — no ghost point. Never blocks drawing. |
...
### 2.10 Canvas ↔ React Data Flow

1. **React → Konva (rendering):** Measurements from React Query rendered as Konva shapes. Positions from `geometry_data`. Colors from condition cache. Visibility filtered by `conditionVisibility`.
2. **Konva → React (events):** User interactions fire React callbacks → update Zustand + React Query mutations → re-render.
3. **Never** read Konva internal state as source of truth. After drag, read position from event, update React state.
4. **Z-Order:** Areas (bottom) → Lines → Points → Selected (top) → Drawing preview (topmost). AI ghosts render above plan image but below user measurements.
...
- **Deliverables:**
  - `frontend/src/stores/workspaceStore.ts` — full store with all slices from §2.5
  - `frontend/src/lib/constants.ts` — all constants from §2.9
  - `frontend/src/types/workspace.ts` — DrawingTool, FocusRegion, Point, PredictedPoint, ClipboardEntry types
  - `frontend/src/lib/FocusContext.tsx` — focus region tracking (see below)
- **FocusContext System:**
...
- **Acceptance Criteria:**
  ...
  - [ ] Constants file covers all values from §2.9
  - [ ] TypeScript types compile without errors
  - [ ] FocusContext correctly identifies active region
  - [ ] All state invariants from §2.5 are enforced in actions
...
|------|----------|--------|--------|
| A.0 State architecture + FocusContext | P0 | 1 day | ⬜ |
...
- **Priority:** P0 (Critical)
- **Estimated Effort:** 3 days (revised from 2 — async server sync adds complexity)
- **Description:** Async command pattern for all canvas actions with server synchronization
- **Command Types:**
  - `DrawMeasurement` — execute: POST to server; undo: DELETE; redo: POST (new ID)
  - `DeleteMeasurement` — execute: DELETE, store full data; undo: POST (re-create); redo: DELETE
  - `MoveMeasurement` — execute: PATCH new geometry; undo: PATCH old geometry; redo: PATCH new
  - `EditMeasurement` — execute: PATCH; undo: PATCH previous; redo: PATCH new
  - `ChangeCondition` — execute: PATCH condition_id; undo: PATCH original; redo: PATCH new
- **Technical Details:**
  ...
  - **Commands are async** — execute/undo/redo return Promises
...
- **UndoManager Interface:**
  ```typescript
  interface Command {
    execute(): Promise<void>;    // async — server calls
    undo(): Promise<void>;
...
- **Priority:** P0 (Critical)
- **Estimated Effort:** 2 days (revised from 1.5 — includes hover, z-order, selection visuals)
...
- **Selection Visual Constants:**
  ...
  - Selected stroke: 2× normal width, dashed (`dashArray: [8, 4]`)
  - Selected fill: original fill + 0.1 opacity boost
- **Z-Order Rendering (enforced in MeasurementLayer):**
  - Area measurements (polygons, rectangles) — bottom
  - Linear measurements (lines, polylines) — middle
  - Count measurements (points) — top
  - Selected measurement(s) — always topmost, regardless of type
  - In-progress drawing — above everything
- **Technical Details:**
...
| Task | Priority | Effort | Status |
|------|----------|--------|--------|
| C.1 Undo/Redo system (async + server sync) | P0 | 3 days | ⬜ |
| C.2 Tool keyboard shortcuts | P0 | 1 day | ⬜ |
| C.3 Live measurement preview | P0 | 1.5 days | ⬜ |
| C.4 Snap-to-endpoint | P0 | 1 day | ⬜ |
| C.5 Select/Edit/Move + hover + z-order | P0 | 2 days | ⬜ |
| C.6 Multi-select | P1 | 1 day | ⬜ |
| C.6.5 Copy/Paste measurements | P1 | 1 day | ⬜ |
...
| C.11.5 Area fill patterns | P2 | 0.5 days | ⬜ |
| C.11.7 Zoom to selection | P2 | 0.5 days | ⬜ |
