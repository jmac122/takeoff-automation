...
class ExcelExporter(BaseExporter):
    ...
    def _build_summary_sheet(self, wb: Workbook, data: ExportData) -> None:
        ...
        # Title row
        ws.cell(row=1, column=1, value=f"Export: {data.project_name}").font = Font(bold=True, size=14)
        if data.client_name:
            ws.cell(row=2, column=1, value=f"Client: {data.client_name}")
        ...
        for idx, cond in enumerate(data.conditions):
            row = start_row + 1 + idx
            ws.cell(row=row, column=1, value=cond.name)
            ws.cell(row=row, column=2, value=cond.measurement_type)
            ...
            ws.cell(row=row, column=6, value=cond.category or "")
            ws.cell(row=row, column=7, value=cond.scope)
    ...
    def _build_detail_sheets(self, wb: Workbook, data: ExportData) -> None:
        ...
        for cond in data.conditions:
            ...
            ws.cell(row=1, column=1, value=cond.name).font = Font(bold=True, size=12)
            ws.cell(row=2, column=1, value=f"Type: {cond.measurement_type}  |  Unit: {format_unit(cond.unit)}  |  Total: {cond.total_quantity:.2f}")
            ...
            for m_idx, m in enumerate(cond.measurements):
                ...
                ws.cell(row=row, column=7, value=m.notes or "")
    ...
    def _build_page_sheets(self, wb: Workbook, data: ExportData) -> None:
        ...
        for page_num in sorted(pages.keys()):
            for m in pages[page_num]:
                ...
                ws.cell(row=current_row, column=3, value=m.condition_name)
